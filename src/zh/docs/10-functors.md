...
因此，解决这个抽象问题意味着找到一种**以通用方式组合程序**的具体方法。这对于我们开发者来说真的很有趣，不是吗？
...
该技术需要一种组合作用的方法，以及定义一个解释器，该解释器能够在启动最终程序时执行副作用。
...
在函数式编程中，倾向于将副作用（以作用的形式）推到系统边界（`main`函数），在那里由解释器执行它们，由此可以得到以下方案：

> system = pure core + imperative shell

在 _纯函数式语言_ （如 Haskell、PureScript或Elm）中，这种划分是严格而清晰的，并且是由语言本身强加的。

即使使用这种thunk技术（这正是`fp-ts`采用的技术），我们也需要一种方法来组合作用，这使我们回到了以通用方式组合程序的目标，让我们看看如何实现。
...
更一般地，当类型构造函数`F`拥有`map`函数时，我们说它拥有一个**函子实例**。
