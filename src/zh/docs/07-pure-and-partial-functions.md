...
这与我们在TypeScript中定义的`double`函数非常类似。
...
注意，为了组合`f`和`g`，`f`的定义域必须包含于`g`的到达域中。

**定义**：偏函数是没有为定义域中所有值定义的函数。

相反，为定义域中所有值定义了的函数称为全函数。
...
在函数式编程中，我们倾向于只定义**纯函数和全函数**（从现在开始，“函数”将专指“纯函数和全函数”），当我们碰到偏函数时，我们应该怎么办？
