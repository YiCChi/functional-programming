...
したがって、この抽象的な問いに対する答えを見つけることは、**プログラムを汎用的に合成する具体的な方法** を見つけることを意味します。そして、それは私たち開発者にとって非常に興味深いことではないでしょうか？
...
```ts
type DSL = ... // システムで処理されうるすべての effect の直和型
...
この thunk 手法（`fp-ts` で使用されているのと同じ手法）を用いた場合でも、effect を結合する方法が必要です。これは、プログラムを汎用的な方法で合成するという目標に戻ることを意味します。それでは、どのようにするかを見てみましょう。
...
このようなシグネチャは、入力を型 `A` とし、型 `B` の結果を effect `F` とともに返すプログラムをモデリングしています。ここで `F` は一種の型コンストラクタです。
...
現在のルールだけでは、この一般的な問題を解決することはできません。`B` と `C` にいくつかの「制約」を追加する必要があります。
...
- 圏 _C_ の任意の対象 `X` を、圏 _D_ 内の対象に対応付けする **対象の写像**
...
- `F` は、任意の型 `X` を型 `F<X>` に写像する `n` 項（`n >= 1`）の型コンストラクタです（**対象の対応付け**）
