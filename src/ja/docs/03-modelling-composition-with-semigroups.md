...
`Magma<A>`はとても単純な代数的構造です。

- 集合または型（`A`）
- `concat`演算
- 守るべき法則はなし

**注**：ほとんどの場合、_集合（set）_ と _型（type）_ という用語は互換的に使用できます。
...
- 集合`A`
- 集合`A`の上の二項演算`concat`。この演算は集合`A`において **閉じている（closed）** と言います。つまり、`A`の任意の元`a`、`b`に対して、この演算の結果は再び`A`に属する。結果は依然として`A`の元であるため、再び`concat`の入力として使用でき、この演算を好きなだけ繰り返すことができます。言い換えれば、`concat`は`A`型の`コンビネータ`です。
...
**定義**：空でない集合`A`と`A`において閉じている二項演算`*`の対`(A, *)`を **亜群(またはマグマ、magma)** と呼ばれている。
...
**クイズ**：普通に`concat`は[**交換法則**](https://en.wikipedia.org/wiki/Commutative_property)を満たさないため、このコンビネータは意味をなしています。交換法則に満たす`concat`の例とそうでない例を見つけられますか？
